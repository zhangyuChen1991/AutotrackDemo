...
class PluginEntry implements Plugin<Project> {
    ...
    void apply(Project project) {
        project.extensions.create('AutoTrackConfig', AutoSettingParams)
        ...
        project.afterEvaluate {
            Logger.setDebug(project.AutoTrackConfig.isDebug)
            // 用户配置解析
            analysiUserConfig()
        }
    }
    ...
    static void analysiUserConfig() {
        ...
        String trackClass = GlobalConfig.getParams().trackClass
        LogHookConfig.LOG_ANALYTICS_BASE = trackClass
